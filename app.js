...
(function(){
  ...
  function renderCategory(host, cat){
    ...
    (cat.items||[]).forEach(it=>{
      ...
      if(it.phone){ const aTel = document.createElement('a'); aTel.href = telHref(it.phone); aTel.className='btn btn-ghost'; aTel.textContent = it.phone; aTel.setAttribute('aria-label','Chiama ' + it.name); row.appendChild(aTel); }
      const aPref = document.createElement('a'); aPref.href = mapUrl(it.address, it.name); aPref.target='_blank'; aPref.rel='noopener'; aPref.className='btn'; aPref.textContent='Apri in Mappe';
      const aG = document.createElement('a'); aG.href = 'https://www.google.com/maps/search/?api=1&query=' + encodeURIComponent(it.address || it.name); aG.target='_blank'; aG.rel='noopener'; aG.className='btn'; aG.textContent='Google';
      const aA = document.createElement('a'); aA.href = 'https://maps.apple.com/?q=' + encodeURIComponent(it.address || it.name); aA.target='_blank'; aA.rel='noopener'; aA.className='btn'; aA.textContent='Apple';
...
(function(){
  const link = document.getElementById('eatMenuLink');
  const drawer = document.getElementById('drawer');
  const btn = document.getElementById('menuBtn');
  if(link){
