...
(function(){
  ...
  function renderCategories(host){
    ...
    EAT_DATA.categories.forEach(cat=>{
      ...
      const p = document.createElement('p'); p.className='muted'; p.style.margin='0 0 8px 0'; p.textContent = 'Seleziona per vedere i ristoranti';
      const btn = document.createElement('a'); btn.href='#'; btn.className='btn'; btn.textContent='Apri categoria'; btn.addEventListener('click', (e)=>{e.preventDefault(); renderCategory(host, cat);}); card.appendChild(h); card.appendChild(p); card.appendChild(btn);
  ...
  function makeToolbar(host, title, onBack){
    ...
    const back = document.createElement('a'); back.href='#'; back.className='btn outline'; back.textContent='← Indietro'; back.addEventListener('click', (e)=>{e.preventDefault(); onBack();});
  ...
  function renderCategory(host, cat){
    host.innerHTML = '';
    makeToolbar(host, cat.name, ()=>renderCategories(host));
    ...
    (cat.items||[]).forEach(it=>{
      ...
      if(it.phone){ const aTel = document.createElement('a'); aTel.href = telHref(it.phone); aTel.className='btn'; aTel.textContent = 'Chiama'; aTel.setAttribute('aria-label','Chiama ' + it.name); row.appendChild(aTel); }
      const aPref = document.createElement('a'); aPref.href = mapUrl(it.address, it.name); aPref.target='_blank'; aPref.rel='noopener'; aPref.className='btn'; aPref.textContent='Apri in Mappe'; aPref.setAttribute('data-emoji','map');
...
(function(){
  ...
  function renderDoCategories(host){
    ...
    DO_DATA.categories.forEach(cat=>{
      ...
      const h = document.createElement('h3'); h.textContent=cat.name; h.style.margin='8px 0';
      const p = document.createElement('p'); p.className='muted'; p.style.margin='0 0 8px 0'; p.textContent='Seleziona per vedere le attività';
  ...
  function makeDoToolbar(host, title, onBack){
    ...
    bar.style.gap='8px'; bar.style.margin='0 0 8px 0';
    const left = document.createElement('div');
    const back = document.createElement('a'); back.href='#'; back.className='btn outline'; back.textContent='← Indietro';
  ...
  function renderDoCategory(host, cat){
    host.innerHTML = '';
    makeDoToolbar(host, cat.name, ()=>renderDoCategories(host));
    ...
    (cat.items||[]).forEach(it=>{
      ...
      const row = document.createElement('div'); row.style.display='flex'; row.style.flexWrap='wrap'; row.style.gap='8px'; row.style.marginTop='6px';
      const aPref = document.createElement('a'); aPref.href = mapUrlDo(it.address, it.name); aPref.target='_blank'; aPref.rel='noopener'; aPref.className='btn'; aPref.textContent='Apri in Mappe';
      const aG = document.createElement('a'); aG.href = 'https://www.google.com/maps/search/?api=1&query=' + encodeURIComponent(it.address || it.name); aG.target='_blank'; aG.rel='noopener'; aG.className='btn'; aG.textContent='Google'; aG.setAttribute('data-emoji','google');
      const aA = document.createElement('a'); aA.href = 'https://maps.apple.com/?q=' + encodeURIComponent(it.address || it.name); aA.target='_blank'; aA.rel='noopener'; aA.className='btn'; aA.textContent='Apple'; aA.setAttribute('data-emoji','apple');
      row.appendChild(aPref); row.appendChild(aG); row.appendChild(aA);
