  ...
  function generatePDF(ev){
    ...
    try{
      ...
      .then(function(arr){
        ...
        var prov=(arr[1].categories.find(c=>c.slug==='provare')||{}).items||[];
        var food=(arr[2].categories.find(c=>c.slug==='tradizione')||{}).items||[]; if(!food.length) food=(arr[2].categories[0]||{}).items||[];
        var meteo=arr[3]||[];
        var logo=arr[4];
