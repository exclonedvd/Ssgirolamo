 ...
 * Usage:
 *   <section id="bacheca"></section>
 *   <script src="/assets/bacheca_widget.js" data-target="#bacheca" data-title="Bacheca ospiti"></script>
 */
(function(){
  try{
    ...
    var DATA = [
      ...
      { t:'Crociera tra i laghi, Mincio e fiori di loto', k:'provare', m:'https://www.google.com/maps?q=Crociera+laghi+Mantova+fiori+di+loto' },
      { t:'I Barcaioli del Mincio', k:'provare', m:'https://www.google.com/maps?q=Barcaioli+del+Mincio' },
      { t:'Degustazione in caseificio (Latteria S. Pietro, Goito)', k:'provare', m:'https://www.google.com/maps?q=Latteria+San+Pietro+Goito' },
    ...
    // crea wrapper e UI minimale che eredita lo stile del sito
    el.classList.add('bacheca-widget');
    el.innerHTML = [
      '<div class="bw-header"><h2>'+titleText+'</h2>',
      '<div class="bw-legend">',
    ...
    var css = [
      '.bacheca-widget{--bw-border: rgba(0,0,0,.08);--bw-bg:#fff;--bw-text:inherit;--bw-muted:rgba(0,0,0,.55);}',
      ...
      '.bacheca-widget .bw-tag{display:inline-flex;align-items:center;gap:.5rem;padding:.35rem .6rem;border-radius:999px;border:1px solid var(--bw-border);background:var(--bw-bg);font-size:.9em}',
      ...
      '.bacheca-widget .bw-scroller{position:relative;overflow:hidden;border:1px solid var(--bw-border);border-radius:14px;background:var(--bw-bg)}',
      ...
      '.bacheca-widget .bw-scroller::before{left:0;background:linear-gradient(to right,var(--bw-bg),rgba(255,255,255,0))}',
      '.bacheca-widget .bw-scroller::after{right:0;background:linear-gradient(to left,var(--bw-bg),rgba(255,255,255,0))}',
      ...
      '.bacheca-widget .bw-item{display:inline-flex;align-items:center;gap:.6rem;padding:.5rem .75rem;border-radius:999px;border:1px solid var(--bw-border);background:var(--bw-bg);white-space:nowrap}',
      ...
      '@keyframes bw-autoscroll{from{transform:translateX(0)}to{transform:translateX(var(--bw-to,-50%))}}'
    ].join('');
    var style = document.createElement('style'); style.setAttribute('data-bacheca-widget',''); style.textContent = css;
    document.head.appendChild(style);

    // render items (duplicati per loop infinito)
    var strip = el.querySelector('.bw-strip');
    function makeNodes(){
    ...
    // calcola durata in base alla larghezza (circa 100px/sec)
    requestAnimationFrame(function(){
      var total = strip.scrollWidth/2; // metà, è duplicato
      var speed = 100;
    ...
    console.error('Bacheca widget error:', e);
  }
})();
